...
function Profile({ navigation, route }) {
  ...
  const findYouAndIObject = roomsData?.seeRooms?.find((room) =>
    room.users.some((item) => item.username === username)
  );
  const findYouAndIRoomNumber = findYouAndIObject?.id;
  console.log("room number", findYouAndIRoomNumber);
  ...
  const HeaderRightMessage = () => (
    ...
    >
      <MessageIcon name="paper-plane" size={20} />
    </TouchableOpacity>
  );
